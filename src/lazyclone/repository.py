...
from .console import *
from .git import github_username, github_repositories


def choose_repository(choices: list[str]) -> str:
    if len(choices) == 0:
        raise "No repositories to choose from"
...
def resolve_repo(repo: str) -> str:
    ...
    return choose_repository(choices)

    # Resolve repository name only
    if re.match("^[a-zA-Z0-9._-]+$", repo):
        username = github_username()
        if username is None:
            raise "Failed to get GitHub username"
    ...
    raise "Unable to resolve repository"
